...
func parseGo(workspaceDir string) (parseResult, error) {
	manifestPath := filepath.Join(workspaceDir, "go.mod")
	...
	result := parseResult{ManifestFound: true, ManifestPath: "go.mod"}

	sumDeps, err := parseGoSum(filepath.Join(workspaceDir, "go.sum"), direct)
	if err != nil {
		if os.IsNotExist(err) {
			out := make([]Dependency, 0, len(direct))
			for name, version := range direct {
				out = append(out, Dependency{Ecosystem: "go", Name: name, Version: version, IsDirect: true, ManifestPath: "go.mod", DeclaredRange: version})
			}
...
func parseGoSum(path string, direct map[string]string) ([]Dependency, error) {
	f, err := os.Open(path)
	if err != nil {
		return nil, err
	}
	...
	for scanner.Scan() {
		...
		deps = append(deps, Dependency{
			...
			IsDirect: isDirect, ManifestPath: "go.sum", DeclaredRange: direct[name],
		})
	}
	if err := scanner.Err(); err != nil {
