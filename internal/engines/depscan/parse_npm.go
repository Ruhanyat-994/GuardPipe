...
// package.json under backend/ or frontend/ in a monorepo checkout is just
// as real a manifest as one at the top level, and each directory found
// produces its own parseResult (documentation/05-module-specifications.md
// §4: depscan must "recursively discover manifests anywhere in the
// checkout," not only at the root).
func parseNPM(workspaceDir string) ([]parseResult, error) {
	dirs, err := findManifestDirs(workspaceDir, "package.json")
	if err != nil {
		return nil, fmt.Errorf("depscan: find package.json: %w", err)
	}
	results := make([]parseResult, 0, len(dirs))
	for _, dir := range dirs {
		result, err := parseNPMAt(workspaceDir, dir)
		if err != nil {
			return nil, err
		}
...
func parseNPMAt(workspaceDir, dir string) (parseResult, error) {
	...
	if err != nil {
		...
		result.Dependencies = directOnlyNPM(direct, relManifest)
		return result, nil
	}
	...
	for pkgKey, entry := range lock.Packages {
		...
		deps = append(deps, Dependency{
			...
			IsDirect: isDirect, ManifestPath: relLock, DeclaredRange: directRangeOrEmpty(isDirect, name, pkg),
		})
	}
...
func directOnlyNPM(direct map[string]string, manifestPath string) []Dependency {
	out := make([]Dependency, 0, len(direct))
	for name, r := range direct {
		out = append(out, Dependency{
			Ecosystem: "npm", Name: name, Version: strings.TrimLeft(r, "^~>=< "),
			IsDirect: true, ManifestPath: manifestPath, DeclaredRange: r,
		})
	}
	return out
}
