...
func parsePyPI(workspaceDir string) (parseResult, error) {
	manifestPath := filepath.Join(workspaceDir, "requirements.txt")
	...
	for scanner.Scan() {
		...
		deps = append(deps, Dependency{
			...
			IsDirect: true, ManifestPath: "requirements.txt", DeclaredRange: operator + version,
		})
	}
	...
	return parseResult{
		Dependencies: deps, ManifestFound: true, ManifestPath: "requirements.txt",
