...
export function AdminSystemHealthPage() {
  ...
  function refresh() {
    getSystemHealth()
      .then(setHealth)
      .catch((err: unknown) => {
        setError(err instanceof ApiError ? err.problem.detail : 'Could not load system health.')
      })
  }
  ...
  return (
    <main className="mx-auto max-w-5xl px-6 py-8">
      ...
      {!health && !error && <p className="text-body-sm text-text-secondary">Loading…</p>}
        ...
        <>
          <div className="mb-6 grid grid-cols-2 gap-4 sm:grid-cols-4">
            ...
            <MetricCard
              ...
              caption={health.sandbox_containers_running === null ? 'Unavailable' : 'Running now'}
            />
            <MetricCard
          ...
          <Card>
            <CardTitle className="text-h3">Engine job outcomes (last 24h)</CardTitle>
                  ...
                  <tbody className="divide-y divide-border-default">
                    {health.engine_stats.map((stat) => (
                      <tr key={stat.engine}>
                        ...
                        <td className="py-2.5 pr-4 text-success">{stat.succeeded}</td>
                        <td className="py-2.5 pr-4 text-danger">{stat.failed}</td>
                        <td className="py-2.5 text-text-tertiary">{stat.skipped}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
