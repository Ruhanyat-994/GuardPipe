...
export function listOrganizations(search = ''): Promise<OrganizationList> {
  ...
  return apiClient.get<OrganizationList>(`/admin/organizations${query ? `?${query}` : ''}`)
}

export function getOrganization(id: string): Promise<OrganizationDetail> {
  return apiClient.get<OrganizationDetail>(`/admin/organizations/${encodeURIComponent(id)}`)
}

export function suspendOrganization(id: string, reason: string): Promise<OrganizationDetail> {
...
export function suspendUser(id: string, reason: string): Promise<void> {
  return apiClient.post<void>(`/admin/users/${encodeURIComponent(id)}/suspend`, { reason })
}

export function reinstateUser(id: string): Promise<void> {
  return apiClient.post<void>(`/admin/users/${encodeURIComponent(id)}/reinstate`)
}

// --- pentest misuse flags ---

export type FlagStatus = 'open' | 'investigating' | 'dismissed' | 'confirmed_misuse'
export type FlagSource = 'self_reported' | 'external_complaint' | 'operator_review'
...
export function listPentestFlags(status?: FlagStatus): Promise<PentestFlagList> {
  const params = new URLSearchParams()
  if (status) params.set('status', status)
  params.set('page_size', '100')
  const query = params.toString()
  return apiClient.get<PentestFlagList>(`/admin/pentest-flags${query ? `?${query}` : ''}`)
}
...
export function listAuditLog(
  ...
  const query = params.toString()
  return apiClient.get<AuditLogList>(`/admin/audit-log${query ? `?${query}` : ''}`)
}
